...
보시다시피 `$`와 `+`는 다른 의미를 가지며, `+`는 스트림 내 가능한 가장 큰 ID를 의미하고, `$`는 특정 스트림에서 현재 존재하는 항목 중 가장 큰
...
이는 스트림이 소비자 그룹과 연결되어 있기 떄문이며, 항목이 더 이상 없다고 해서 소비자 그룹의 상태까지 잃는 것을 원하지 않기 때문입니다. 또한,
